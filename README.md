...
- **2026-09-17（複核，無規則異動）**：依排程任務再次核對 NICS（國家資通安全研究院）現行公告之 GCB 版本。
  - 本工作階段直接連線 `www.nics.nat.gov.tw` / `download.nics.nat.gov.tw` 仍為 `EGRESS_BLOCKED`（環境網路政策封鎖，非暫時性問題），改以公開網路搜尋（含 `site:download.nics.nat.gov.tw` 網域限定查詢）交叉比對官方文件檔名確認版本，方法與歷次複核一致。
  - 確認 **`TWGCB-01-012`（Red Hat Enterprise Linux 9 政府組態基準說明文件，伺服器）現行仍為 v1.2（中華民國114年6月12日 / 1140612）**，與 **`TWGCB-04-007`（Apache HTTP Server 2.4）v1.2** 一致，與 2026-07-17（v2.2）、2026-08-19、2026-09-10、2026-09-12～09-16 歷次複核結論相同，未發現官方已發布新版本。
  - 額外查證 NICS 網站「115年政府組態基準GCB說明文件」相關報導：**Apple macOS 15.x（TWGCB-01-015）已於 1150626 正式發布 v1.0**（不再是預告版），另 Fortinet FortiGate（TWGCB-03-006）仍為預告項目；兩者均與 Rocky Linux 9 / RHEL 9 或 Apache HTTP Server 2.4 無關，故本次**不**調整任何規則期望值或版本標示。
...
- **2026-09-12（複核，無規則異動）**：依排程任務再次核對 NICS（國家資通安全研究院）現行公告之 GCB 版本。
  ...
  - 進一步查證 NICS 網站「115年政府組態基準GCB說明文件（預告版）」頁面（`/core_business/cybersecurity_defense/GCB/gcbreview/`）之公開報導內容，115年度新增項目為 **Apple macOS 15.x（TWGCB-01-015，預告版，約68項）** 與 **Fortinet FortiGate（TWGCB-03-006，預告版）**，均未見 Red Hat Enterprise Linux 9 或 Apache HTTP Server 2.4 相關項目被列入本次預告更新範圍，故本次**不**調整任何規則期望值或版本標示。
  - 因此本次**未修改** `GCB_CHECK/*.sh` 或 `GCB_SET/*.sh` 之任何檢測邏輯或期望值，僅更新本文件之複核紀錄與最後更新日期。
