...
- **2026-07-03 基準版本複查**（回應 issue #4）：依 NICS 官方文件下載索引與多筆獨立網路搜尋結果交叉確認，`TWGCB-01-012`（RHEL 9 伺服器）與 `TWGCB-04-007`（Apache 2.4）**最新公告版本仍為 v1.2**（`TWGCB-01-012` 修訂於中華民國114年6月12日／1140612；同期 `TWGCB-01-008` RHEL 8 為 v1.3）。115 年版目前僅見預告版，尚未正式發布。本專案基準版本標示維持正確，**無需升版**。
  - 本次審查受限於此執行環境之出口網路政策（`nics.nat.gov.tw`、`download.nics.nat.gov.tw` 皆回應 HTTP 403 / host not allowed），未能直接下載官方 PDF 逐項比對內容；版本存在性係以官方文件下載連結檔名索引與多筆公開搜尋結果交叉驗證，而非逐頁核對說明文件全文。
  - ⚠️ **PR 重複情形提醒**：審查時發現本 repo 目前有 **9 個開放中的草稿 PR（#16、#17、#18、#19、#20、#21、#22、#24、#25）**，其中 #16–#22、#24 皆針對同一個落差重複提出修正——即 `GCB_SET/GCB.sh` 已套用、但 `GCB_CHECK/GCB_check.sh` 尚未涵蓋的 v1.2 規則（`check_cron` 函式、`0189`–`0204`、`0221`、`0308`–`0312` 等）。經逐一比對，**PR #24**（2026-06-28，2 次提交）涵蓋範圍最完整，且額外修正了 `0235`/`0238` 的錯誤 ID 對應問題（應為 `0236`／`0239-0240`），建議維護者優先審閱並合併 #24，其餘 #16、#17、#18、#19、#20、#21、#22 內容已被涵蓋，建議關閉以減少混淆。**PR #25** 為純文件版本複查、與 #24 不衝突，可獨立審閱合併。本 PR（本次變更）與上述 PR 皆僅止於文件層級的版本複查說明，未變更任何腳本邏輯。
